...
def load_training_data(directory, tgt_dir, ext, number_of_images , train_test_ratio = 0.8):
    ...
    files = np.load(directory+'LRintp100um50um_fluo21cubic.npy')
    files_tgt = np.load(tgt_dir+ 'HR50um_fluo21cubic.npy')
...
def load_test_data(directory, ext, number_of_images = 100):
    ...
    files = np.load(directory + 'lrFA_test_patch21subj1.npy')
    files = np.expand_dims(np.transpose(files, (3, 0, 1, 2)), axis=4)
...
def load_test_refdata(directory, ext, number_of_images=100):
    # files = load_data_from_dirs(load_path(directory), ext)

    files = np.load(directory + 'hrFA_test_patch21subj1.npy')
    files = np.expand_dims(np.transpose(files, (3, 0, 1, 2)), axis=4)
...
def plot_test_generated_images(output_dir, generator, x_test_lr, x_test_hr, figsize=(5, 5)):
    ...
    cubic_img_lr = denormalize(image_batch_lr)
    ref_img_hr = denormalize(x_test_hr)
    # cubic_img_lr = image_batch_lr
    # ref_img_hr = x_test_hr

    save(output_dir + 'srResFA_test_patch21.npy', generated_image)
    save(output_dir + 'lrFA_test_patch21subj2.npy', cubic_img_lr)
    save(output_dir + 'hrFA_test_patch21subj2.npy', ref_img_hr)
    # for index in range(examples):
    #
    #     #plt.figure(figsize=figsize)
    #
